...
function ShopPage() {
  ...
  const [isInitializing, setIsInitializing] = useState(() => {
    return !localStorage.getItem("shopItems");
  });
  ...
  // Ambil daftar item shop dan saldo currency (SWR Pattern)
  useEffect(() => {
    async function fetchShopData() {
      try {
        ...
        localStorage.setItem("shopCurrency", JSON.stringify(userStats?.currencyBalance ?? 0));
      } catch (error) {
  ...
  return (
    <div className="flex h-dvh flex-col overflow-hidden justify-between bg-primary text-tertiary">
      ...
      {isInitializing ? (
        <LoadingOverlay message={lang === 'id' ? "Memuat shop..." : "Loading shop..."} />
      ) : (
        <>
          {/* Currency Balance */}
