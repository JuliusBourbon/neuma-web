...
function ShopItemCard({ item, isPurchasing, onPurchase, lang = 'id' }) {
  ...
  return (
    <div className="flex w-full max-w-56 flex-col items-center rounded-lg bg-primary p-2">
      {/* Avatar Image */}
      <div className="flex md:h-40 md:w-full items-center justify-center rounded-md bg-tertiary">
        <img
          src={item.imageUrl}
          alt={itemName}
      ...
      <div className="flex min-h-24 w-full flex-col items-center justify-center gap-1 px-2 text-center">
        ...
        <button
          ...
          className={`mt-1 flex min-w-28 items-center justify-center gap-2 rounded-full border-2 px-4 py-1 text-base font-medium transition ${isOwned
        ...
        >
          {isOwned ? (
            <>
              <span>✓</span>
              <span>{lang === 'id' ? "Dimiliki" : "Owned"}</span>
            </>
          ) : isPurchasing ? (
            lang === 'id' ? "Membeli..." : "Purchasing..."
          ) : (
            <>
